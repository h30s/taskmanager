...
const API_BASE = "https://mern-task-api-fsmo.onrender.com/api/tasks";
    ...
    <div style={{ padding: '20px', maxWidth: '600px', margin: 'auto' }}>
      ...
      {editingTask && (
        <form onSubmit={handleEditSubmit} style={{ marginTop: '20px' }}>
          <h4>Editing: {editingTask.title}</h4>
          <input
            type="text"
            value={editTitle}
      ...
      {tasks.map(task => (
        <div key={task._id} style={{ border: '1px solid #ccc', padding: '10px', margin: '10px 0' }}>
          ...
          <button onClick={() => handleDelete(task._id)} style={{ marginLeft: '10px', color: 'red' }}>
            Delete
          </button>
